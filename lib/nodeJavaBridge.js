...
if (fs.existsSync(path.resolve(__dirname, '../build/jvm_dll_path.json'))) {
...
if (!binaryPath) {
  if (process.env.NODE_JAVA_BRIDGE_BINARY_PATH) {
    binaryPath = process.env.NODE_JAVA_BRIDGE_BINARY_PATH;
  } else {
...
if (fs.existsSync(path.resolve(__dirname, "../commons-lang3-node-java.jar"))) {
  java.classpath.push(path.resolve(__dirname, "../commons-lang3-node-java.jar"));
}
if (fs.existsSync(path.resolve(__dirname, __dirname, "../src-java"))) {
